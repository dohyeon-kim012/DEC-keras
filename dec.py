...
class DECModel(model.MXModel):
    ...
    def setup(self, X, num_centers, alpha, save_to='dec_model'):
        ...
        X_train = X[:sep]
        X_val = X[sep:]
        ae_model = AutoEncoderModel(self.xpu, [X.shape[1], 500, 500, 2000, 5], pt_dropout=0.2)
...
def mnist_exp(xpu):
    X, Y, raw_data, tokens = data.load_crawling_data()
    dec_model = DECModel(xpu, X, 10, 1.0, 'test')
    acc = []
    for i in [10*(2**j) for j in range(9)]:j
